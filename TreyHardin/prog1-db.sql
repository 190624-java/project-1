...
Create Table Reimbursements (
    reimbursment_id Number(8) Primary Key,
...
Create Table Receipts (
    ...
    receipt Blob,
    reimbursment_id Number(8),
    Foreign Key(reimbursment_id) References Reimbursements(reimbursment_id)
);
/
...
FOR EACH ROW
BEGIN
    SELECT rem_id_seq.nextval INTO :new.reimbursment_id
    FROM dual;
END;
/
...
FOR EACH ROW
BEGIN
    SELECT rec_id_seq.nextval INTO :new.receipt_id
    FROM dual;
END;
/

INSERT INTO employees(username, password, email) Values ('TreyHardin', '123', 'thardin@test.com');
commit;